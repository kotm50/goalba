...
function ExpirationOneMinuteModal(props: ExpirationModalProps) {
  const { onClose } = props;
  ...
  const handleReLoginButtonClick = () => {
    jobsiteCommonReissuAccessToken().then((response) => {
      if (response?.code === "C000") {
        ...
        dispatch({ type: "RESET_ONE_MINUTE_MODAL" }); // 1분 모달 초기화
      } else {
      ...
      <div className="relative z-[1001] flex h-auto w-80 flex-col justify-between rounded-[10px] bg-white p-5">
        <div className="flex w-full flex-col gap-5">
          ...
          <div className="flex flex-col gap-1">
            <p className="text-sm">로그인 만료까지 1분 남았습니다.</p>
...
export default ExpirationOneMinuteModal;
